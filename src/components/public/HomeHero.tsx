  ...
  return (
    <section className="relative overflow-hidden bg-espresso text-ivory">
      <div className="grid lg:min-h-[calc(100svh-4.75rem)] lg:grid-cols-2">
        {/* eslint-disable-next-line shadcn/no-arbitrary-values -- full-bleed alignment to the 72rem column */}
        <div className="order-2 flex items-center px-4 py-8 sm:px-6 lg:order-1 lg:py-16 lg:pr-8 lg:pl-[max(1.5rem,calc((100vw-72rem)/2+1.5rem))]">
          <div className="max-w-xl">
            <p className="text-sm uppercase tracking-lux-wide text-champagne">Bayraklı · Manavkuyu</p>
            <h1 className="mt-4 font-serif text-4xl leading-display text-ivory sm:text-6xl">
        ...
        <div className="order-1 flex flex-col lg:order-2 lg:min-h-full lg:flex-row">
          {hasPortrait ? (
            <>
              <div
                aria-hidden
                className="hidden w-16 shrink-0 bg-gradient-to-r from-espresso to-photo-ink sm:w-20 lg:block"
              />
              <div className="relative h-[min(46svh,22rem)] w-full bg-photo-ink lg:h-auto lg:min-h-full">
                <Image
                  src={portraitSrc}
                  alt={alt}
                  fill
              ...
              <div aria-hidden className="h-12 shrink-0 bg-gradient-to-b from-photo-ink to-espresso lg:hidden" />
            </>
          ) : (
